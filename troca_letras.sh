...
echo ""
	case $OPCAO in
		1)
			...
			echo ""
			find "${DIR}" -depth -execdir rename -v -n "s/à/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando á por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v -n "s/á/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ã por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v -n "s/ã/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando â por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v -n "s/â/a/g" {} \;
			echo ""
		...
		2)
			...
			echo -e "${YELLOW_LIGHT}Trocando à por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/à/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando á por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/á/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ã por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ã/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando â por a:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/â/a/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando é por e:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/é/e/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ê por e:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ê/e/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando í por i:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/í/i/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ó por o:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ó/o/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando õ por o:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/õ/o/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ô por o:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ô/o/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ú por u:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ú/u/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ü por u:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ü/u/g" {} \;
			echo ""
			echo -e "${YELLOW_LIGHT}Trocando ç por c:${ENDCOLOR}"
			echo ""
			find "${DIR}" -depth -execdir rename -v "s/ç/c/g" {} \;
			echo ""
